const { getSlotsByDoctorId, addSlot, updateSlot, deleteSlot } = require('../services/availabilityService');
const { getDoctorByUserId } = require('../services/doctorService');
...
const getMyAvailability = async (req, res) => {
  try {
    ...
    const slots = await getSlotsByDoctorId(doctor.doctor_id);
    res.json({ slots });
...
const addMySlot = async (req, res) => {
  try {
    ...
    const slot = await addSlot(doctor.doctor_id, { day_of_week, start_time, end_time, max_appointments });
...
const updateMySlot = async (req, res) => {
  try {
    const doctor = await getDoctorByUserId(req.user.id);
    if (!doctor) return res.status(404).json({ message: 'Doctor profile not found' });
    const slot = await updateSlot(req.params.slotId, doctor.doctor_id, req.body);
...
const deleteMySlot = async (req, res) => {
  try {
    const doctor = await getDoctorByUserId(req.user.id);
    if (!doctor) return res.status(404).json({ message: 'Doctor profile not found' });
    const deleted = await deleteSlot(req.params.slotId, doctor.doctor_id);

...
const listDoctors = async ({ specialty, search, verified } = {}) => {
  ...
  if (verified !== undefined) { conditions.push(`d.verified = $${i++}`); values.push(verified); }
  if (specialty) { conditions.push(`d.specialty ILIKE $${i++}`); values.push(`%${specialty}%`); }
  if (search) {
    conditions.push(`(d.full_name ILIKE $${i} OR d.specialty ILIKE $${i} OR d.hospital_affiliation ILIKE $${i++})`);
    values.push(`%${search}%`);
  }
...
const createDoctor = async ({ user_id, full_name, specialty, bio, license_number, qualification,
  ...
  const result = await pool.query(
    `INSERT INTO doctors (user_id, full_name, specialty, bio, license_number, qualification,
     ...
     RETURNING *`,
    [user_id, full_name, specialty, bio || null, license_number || null, qualification || null,
     experience_years || 0, consultation_fee || 0, phone_number || null,
     languages || 'English', hospital_affiliation || null]
  );
  return result.rows[0];
};

const updateOwnProfile = async ({ user_id, full_name, specialty, bio, license_number,
  qualification, experience_years, consultation_fee, phone_number, profile_image_url,
  ...
  const result = await pool.query(
    `UPDATE doctors SET
     ...
     WHERE user_id = $1
     RETURNING *`,
    [user_id, full_name||null, specialty||null, bio||null, license_number||null,
     qualification||null, experience_years||null, consultation_fee||null,
     phone_number||null, profile_image_url||null, languages||null, hospital_affiliation||null]
  );
  return result.rows[0] || null;
};
...
module.exports = { listDoctors, getDoctorById, getDoctorByUserId, createDoctor, updateOwnProfile, verifyDoctor, getSpecialties };
